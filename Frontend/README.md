# AskRabbi frontend

The production frontend shell is a React 19, TypeScript, Tailwind CSS 4, and Vite 8 application. It currently implements:

- A responsive login screen whose email, Google, and account-creation actions begin the backend-owned WorkOS/AuthKit flow.
- Cookie-authenticated session hydration from the .NET API; no WorkOS secrets or provider tokens enter the Vite bundle.
- A responsive conversation dashboard backed by owner-scoped API data.
- A weekly-learning destination separated from recent conversations in the sidebar; its React module and current-or-latest API request load only when opened.
- Server-generated weekly-teaching recordings with authenticated MP3 streaming, play/pause, seeking, playback speed, and English/Hebrew word seeking and text following. No speech model or synthesis runs on the phone.
- Working create, load, rename, source-update, message-save, and confirmed-delete operations.
- Per-conversation source controls for the same approved logical sources used by the .NET prototype: Torah, Tanakh, Mishnah, Talmud, Rif, Mishneh Torah, Shulchan Arukh with Rema, Zohar, Zohar Chadash, and Mesillat Yesharim.
- Structured answer sources with clickable inline citation numbers, exact validated quotations, direct canonical Sefaria passage links, edition/license metadata, and expandable surrounding context. On wide screens, citations open an independently scrollable source reader beside the chat; on smaller screens, they open a dismissible bottom sheet with the same source navigation.
- An accessible animated grounded-answer status while retrieval, quotation checks, and generation are still running.
- Welcome prompts that cycle on page load and new conversations, then remain static while visible.
- A compact Personalization screen for full name, birth date/time, a reviewed U.S. time-zone selection, response and source-quotation languages, religious background, Jewish heritage or community, and up to 2,000 characters of optional context.
- An active Settings screen with verified account email, WorkOS password-reset request and confirmation flows, exact API-reported usage periods, and Cosmos-backed conversation preferences.
- A profile menu with active Settings, Personalization, and logout actions.
- Desktop sidebar collapse and a mobile navigation drawer.

The browser now uses the .NET API as the authority for the authenticated user, personalization, account preferences, usage, weekly Dvar Torah publication, conversation summaries, source selections, and message history. Requests send `credentials: "include"`; the backend owns and validates the `HttpOnly` application cookie. The weekly module is dynamically imported and fetches `/api/dvar-torah` only after its sidebar destination is selected; a current publication is cached through the next weekly boundary, while pending or fallback responses use a short retry cache. A new-conversation screen remains an unsaved local draft until its first message is submitted. A reviewed first response can return the backend-applied AI title. A first-turn recovery keeps the default title; later responses preserve it unless the user renames the conversation. Only the non-sensitive welcome-prompt index remains session-local.

The message endpoint stores the user turn, retrieves approved evidence, and independently reviews a structured draft by claim kind. Source-backed statements retain exact quotations and trusted source snapshots; reviewed background and uncertainty can have `sources: []`. If evidence is insufficient or validation fails after repair, the API saves an application-written localized recovery reply with no sources. New frontend requests opt into the bounded current-turn response and merge those messages into already loaded context, so long chats do not repeatedly download their complete history; the API's original full-context response remains available for compatibility. Reviewed answers and recovery replies both use the existing `answered` status and normal saved-message rendering, including reload and idempotent retry. Provider, retrieval, cancellation, and quota failures keep their existing handling. The browser never synthesizes an answer or source records. See [answer reliability](../docs/ANSWER_RELIABILITY.md). Every assistant answer includes separate **Copy text** and **Copy with sources** actions with explanatory hover text and independent success or retry feedback. Copy text retains the answer body, including any inline citation numbers. Copy with sources adds a numbered source list with full passage URLs, exact validated quotations, edition/language details, licenses, and stored attribution URLs, so references remain usable when pasted into other apps. Both formats use plain text, including full URLs in the source-inclusive copy, and answers without source snapshots, including background and recovery replies, copy their body without inventing sources. The controls appear on answer hover or keyboard focus on pointer devices and stay visible on touch devices. Inline citations open the exact trusted source in a side-by-side reader at the `xl` breakpoint and in a touch-friendly bottom sheet below it; previous/next controls move among the current answer's sources without disturbing the chat scroll position. Source context starts closed unless the account-backed `showSourceContextByDefault` preference is explicitly enabled; exact quotations and canonical source links remain available either way. The redundant edition-attribution footer is not rendered, while its provenance URL remains in the trusted stored source record. Older off-screen messages use browser content visibility, and assistant rows are memoized to reduce mobile rendering work while an answer arrives.

## Learning questions

The new-conversation screen offers three fixed starter questions: Shabbat candles, chicken with milk, and the Shema. Suggestions do not use the calendar, offline library, current date, or an AI request. Selecting one prepares an editable draft without sending it.

Signed-in visits to `/` open a blank, unsaved `/conversations/new` page without fetching a saved chat. Explicit conversation, calendar, teaching, and settings URLs still restore their selected destination. Successfully deleting a sidebar conversation opens a fresh new-conversation page, even when a different chat was selected; drafts on surviving conversations are preserved. Cancelled or failed deletions leave the current page and draft intact.

Calendar holiday and teaching-source **Ask about this** actions open `/conversations/new` with the reference question ready to edit, without submitting it. They preserve the draft on the previous saved conversation and reset the new chat's source selection. Chat-source follow-ups and starter questions still use the current draft.

Current and archived teachings have a single **Ask about this teaching** action at the top, which attaches the whole teaching to a new editable question. Selecting teaching text remains available for copying but never opens a passage-question toolbar or attaches the selection. Existing conversations with saved passage context remain readable and can be continued.

## Default appearance

The public home/sign-in and password-reset pages start in light mode before the app loads, regardless of the device's appearance or an older account theme cache. Accounts without saved reading preferences also default to light. Signed-in users can choose Light, Dark, or System in Settings & Personalization → Reading; saved choices still apply and persist per account. Public pages and logout never overwrite those choices. The existing reading-preferences API now returns `theme: "light"` when no preferences exist; no schema migration is needed.

## Teaching reading progress

Readers can mark a teaching as read or unread using the control at the end of the opened article. Archive rows show Read/Unread status only. An opened teaching is automatically marked read after more than 33% of its displayed estimated reading time in a visible tab; hidden tabs pause the timer, and leaving the article resets the visit. Estimates use the saved recording's 1× duration, with a 200-words-per-minute text fallback when no valid duration exists. A manual choice stops automatic marking for the rest of that visit. Past teachings includes All teachings / Unread / Read filters that combine with text search and server-side pagination. The filter, search, and page survive article deep links and returning to the archive. Progress is account-backed through `/api/dvar-torah/read-state`, loads independently of the article, and is never stored in the shared offline teaching cache. Saved offline copies remain readable, but changing account progress requires a connection. Automatic saves wait for known account progress and a connection, make at most one attempt per visit, and never toggle an already-read teaching back to unread. Failed saves retain the previous state and can be retried manually.

Chat answer print controls appear on answer hover or keyboard focus on pointer devices, and stay visible on touch devices. The per-answer Focus button is removed; the existing automatic-focus preference and teaching focus control remain available.

## Weekly-teaching audio

The player stays in a bottom dock outside the article scroll area on desktop and mobile, with safe-area spacing for phones. On phones, the player uses two rows with 44px touch controls. Reading-size preferences are unchanged. The teaching stays unhighlighted during playback and word hover. While audio is playing, text following automatically keeps the spoken word in view as it approaches the viewport edge, without coloring it or requiring a toggle. Manual scrolling does not turn following off; the next spoken word brings the reading back into view. Pausing, buffering, or ending playback stops auto-scroll, and resuming playback follows the current word again. Native text selection and visible keyboard focus remain available. The source reader temporarily suspends auto-scroll without interrupting audio. Reduced-motion settings use instant rather than animated repositioning. The page and dock expand on wide screens; teaching actions share a full-width row and wrap with generous spacing on narrow screens. The dock disappears when leaving the article.

The weekly job generates a recording once for each article/voice version after publishing the text. The frontend uses the article's nullable `audio` descriptor to show a compact player; an absent recording never prevents reading. The native audio element preloads when the article opens with `crossOrigin="use-credentials"` and `preload="auto"`, without autoplay. Pressing Listen or selecting a timed word starts playback immediately within the user gesture (important on iOS). Audio is streamed through `/api/dvar-torah/archive/{weekKey}/audio?version={version}`, never directly from private Blob Storage. The API supports byte ranges, so seeking does not require downloading the whole file.

Timing metadata loads alongside the recording when the article opens, from the sibling `/audio/timings` endpoint. The frontend checks the recording version, exact normalized title/body, UTF-16 text positions, and monotonic word times before enabling word seeking and text following. A mismatched or unavailable manifest disables those controls without breaking playback. The audio clock stays in a ref; only changed words update memoized paragraph rendering, and stable word nodes preserve native passage selection. Source-reference buttons stay available while listening. Leaving the article stops playback, releases the media request, and aborts pending timing requests. A publication without audio is cached for only five minutes so a newly completed recording can be discovered.

## Personalization boundary

Personalization collects one reviewed U.S. IANA birth time zone in addition to birth date and time. A time zone establishes regional civil-date context, but it cannot determine the precise local sunset without a location. The frontend intentionally does not calculate or claim a Hebrew birthday: a future backend milestone must request birthplace when sunset precision matters, use a reviewed Hebrew-calendar and sunset calculation, preserve the original input, and return a verifiable result.

Password-reset requests/confirmation, monthly usage, and conversation defaults now come from the backend. The backend enforces the monthly token allowance before paid chat work and records all provider-reported tokens, including unsuccessful drafts; fixed recovery wording adds no model request.

Profile labels guide wording and potentially relevant community distinctions. They do not count as source evidence, establish observance, or authorize the model to stereotype a user. The API owns validation, authorization, and persistence; the later answer endpoint must send only the minimum profile projection required for personalization.

The response-language and quotation-language choices are independent. Both default to English and support English, French, German, Hebrew, Italian, Persian, Polish, Russian, Spanish, and Yiddish in alphabetical display order. A quotation preference is a retrieval preference, not permission to fabricate a translation: source quotations must use verified approved text, with a brief availability notice when the requested edition is unavailable. Reviewed background and fixed recovery replies do not need quotations.

Source filters are conversation-scoped and use the prototype's stable `collection:*` and `work:*` keys. New conversations enable every approved source by default. Individual sources can still be removed, while quick actions restore the four core collections or select the full approved set. Non-empty selections are saved through the conversation API, and the composer disables sending when all sources are cleared so the answer pipeline cannot silently use an unsupported source set.

## Requirements

- Node.js 22.12 or newer; Node.js 24 LTS is recommended.
- pnpm 11.

## Commands

```powershell
cd Frontend
pnpm install
pnpm dev
```

Verification:

```powershell
pnpm verify
```

The Vite development server defaults to `http://localhost:5173`, and development API calls default to `http://localhost:5090`. A production build defaults to `https://api.askarabbi.ai` for the frontend hosted at `https://app.askarabbi.ai`. Override only the public API origin with `VITE_API_BASE_URL`; Vite exposes every `VITE_*` value to browser code, so never place a WorkOS key, MongoDB connection string, or other secret there. Use `pnpm preview` after `pnpm build` to inspect the production bundle locally, and follow the [production deployment plan](../docs/PRODUCTION_DEPLOYMENT.md) for DNS, SPA rewrites, WorkOS, and backend secrets.

## Authentication boundary

Public [Privacy Policy](https://askarabbi.ai/privacy) and [Terms of Service](https://askarabbi.ai/terms) documents are owned and built by `WebsiteFrontend`. The application links to them with absolute website URLs, including section anchors and printed copies; its Cloudflare Pages redirects preserve old policy routes. They remain readable without sign-in or JavaScript. See [legal policy maintenance](../docs/LEGAL_POLICIES.md) for the source files, integrated notices, data-practice evidence, and release checks.

`src/features/auth/AuthProvider.tsx` depends on the narrow `AuthClient` contract in `authTypes.ts`; `backendAuthClient.ts` implements it with the AskRabbi API. Email supplies a WorkOS login hint, Google selects WorkOS Google OAuth directly, and account creation supplies the WorkOS sign-up screen hint. The backend owns every authorization URL, callback, token exchange, password-reset confirmation, user mapping, and secure application session. No WorkOS API key or provider secret belongs in this Vite project. Hermetic UI tests inject in-memory clients from `src/test` instead of contacting external services. See the [production authentication design](../docs/AUTHENTICATION.md).

For a credential-free account and conversation walkthrough, run the API with its explicitly Development-only `local-demo` profile, then run Vite. That mode exercises the real controllers, cookies, ownership rules, and API adapters while storing data only in the API process. Grounded chat additionally needs all `AI:*` settings; when they are omitted, the API remains healthy and returns `ai_unavailable` without calling a model:

```powershell
dotnet run --project Backend/AskARabbi.Api --launch-profile local-demo
cd Frontend
pnpm dev
```

## Voice questions

The composer supports bounded Record question / Stop recording turns with transcript review before Send. Voice questions can receive spoken answers while text and citations stay visible; saved answers also provide Listen to answer. Permission denial and audio failures leave typed chat usable. See [voice setup, privacy, and verification](../docs/VOICE_CHAT.md). No browser Speech credentials or additional frontend dependencies are required.
