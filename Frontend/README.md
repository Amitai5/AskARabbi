...
- Server-generated weekly-teaching recordings with authenticated MP3 streaming, play/pause, seeking, playback speed, and synchronized English/Hebrew word highlighting. No speech model or synthesis runs on the phone.
...
The player stays in a bottom dock outside the article scroll area on desktop and mobile, with safe-area spacing for phones. On phones, the player uses two rows with 44px touch controls, and secondary teaching actions use labeled icons to leave more room for reading. Reading-size preferences are unchanged. Word highlighting automatically follows the reading when it approaches the viewport edge. Wheel, touch-scroll, scrollbar, or reading-area keyboard navigation pauses following; **Follow text** restores it. The source reader temporarily suspends auto-scroll without interrupting audio. Reduced-motion settings use instant rather than animated repositioning. The dock disappears when leaving the article.
...
Timing metadata loads alongside the recording when the article opens, from the sibling `/audio/timings` endpoint. The frontend checks the recording version, exact normalized title/body, UTF-16 text positions, and monotonic word times before highlighting or word seeking. A mismatched or unavailable manifest disables highlighting without breaking playback. The audio clock stays in a ref; only changed words update memoized paragraph rendering. Source-reference buttons stay available while listening. Leaving the article stops playback, releases the media request, and aborts pending timing requests. A publication without audio is cached for only five minutes so a newly completed recording can be discovered.
