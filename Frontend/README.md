...
- Structured answer sources with clickable inline citation numbers, exact validated quotations, direct canonical Sefaria passage links, edition/license metadata, and expandable surrounding context.
...
The browser now uses the .NET API as the authority for the authenticated user, personalization, account preferences, usage, conversation summaries, source selections, and message history. Requests send `credentials: "include"`; the backend owns and validates the `HttpOnly` application cookie. A new-conversation screen remains an unsaved local draft until its first message is submitted. The first successful grounded response returns the backend-applied AI title; later responses preserve that title unless the user explicitly renames it. Only the non-sensitive welcome-prompt index remains session-local.

The message endpoint now stores the user turn, retrieves only from the configured approved corpus, generates a structured draft, validates every citation and quotation, and persists the validated assistant answer together with trusted source snapshots. The UI displays the canonical returned conversation and surfaces typed fail-closed outcomes instead of fabricating or retaining a local answer. Source context starts open or closed according to the account-backed `showSourceContextByDefault` preference; exact quotations and source links remain available either way.
