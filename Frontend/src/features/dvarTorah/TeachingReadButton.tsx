...
export function TeachingReadButton({ progress, weekKey, title, onToggle }: { progress: TeachingReadProgress; weekKey: string; title: string; onToggle?(): void }) {
  ...
  return (
    <button type="button" onClick={() => { onToggle?.(); void progress.toggle(weekKey) }} disabled={progress.offline || progress.keys === null || progress.pendingKey !== null}
