...
export const HighlightedText = memo(function HighlightedText({ text, textOffset = 0, activeWord, words = NoWords, onSelectWord }: HighlightedTextProps) {
  ...
  if (onSelectWord !== undefined && visibleWords.length > 0) {
    ...
    visibleWords.forEach((word, index) => {
      ...
      parts.push(
        <button key={word.textOffset} type="button" data-narration-seek tabIndex={index === focusedIndex ? 0 : -1} title={`Listen from “${word.text}”`} onFocus={() => setFocusedIndex(index)} onKeyDown={(event) => moveWordFocus(event, index)} onClick={(event) => {
        ...
        }} className="inline cursor-pointer select-text rounded-sm text-inherit transition-colors hover:bg-brass/20 focus-visible:bg-brass/20">
          {activeWord === word ? <mark data-narration-word className="rounded-sm bg-brass/30 text-ink shadow-[0_0_0_2px_var(--color-brass)]">{word.text}</mark> : word.text}
        </button>,
      )
...
function moveWordFocus(event: KeyboardEvent<HTMLButtonElement>, index: number) {
  if (!['ArrowLeft', 'ArrowRight', 'Home', 'End'].includes(event.key)) {
    return
  }
  const buttons = event.currentTarget.parentElement?.querySelectorAll<HTMLButtonElement>('[data-narration-seek]')
