...
export function PrintAction({ getRequest, label, iconOnly = false, compact = false, hideLabelOnMobile = false, className = '' }: { getRequest(): PrintRequest; label: string; iconOnly?: boolean; compact?: boolean; hideLabelOnMobile?: boolean; className?: string }) {
  ...
  return <>
    <button type="button" onClick={event => void open(getRequest, event.currentTarget)} disabled={loading} aria-label={label} title={label} className={`inline-flex shrink-0 items-center justify-center gap-2 text-sm font-semibold transition hover:bg-stone hover:text-pomegranate focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-pomegranate disabled:opacity-50 ${buttonClass} ${className}`}>
      {loading ? <LoaderCircle aria-hidden="true" className="size-4 animate-spin" /> : <Printer aria-hidden="true" className="size-4" />}{iconOnly ? null : <span className={hideLabelOnMobile ? 'hidden sm:inline' : undefined}>{label}</span>}
    </button>
