...
export function OfflineLearningStatus() {
  ...
  return <p role="status" className="mt-4 flex items-center gap-2 text-sm leading-6 text-muted">{state.isSaving ? <LoaderCircle aria-hidden="true" className="size-4 shrink-0 animate-spin motion-reduce:animate-none" /> : null}{state.isSaving ? 'Preparing learning for offline use…' : saved ? `Saved on this device: ${saved.audio ? 'text, references, and audio' : 'text and references'}${state.library?.holidays ? ', plus holidays' : ''}.` : 'Sign in on a good connection to prepare your offline learning.'}</p>
}
