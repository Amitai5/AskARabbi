...
export function SettingsSidebar({ section, isMobileOpen, onClose, onBack, onNavigate }: Props) {
  ...
  return <aside ref={panel} role={isMobileOpen ? 'dialog' : undefined} aria-modal={isMobileOpen ? true : undefined} onKeyDown={handleKey} aria-label="Settings navigation" className={`fixed inset-y-0 left-0 z-40 flex h-dvh min-h-0 w-[min(22rem,calc(100vw-3rem))] shrink-0 flex-col overflow-hidden border-r border-line bg-stone text-base leading-6 text-ink transition-transform lg:relative lg:visible lg:w-[22rem] lg:translate-x-0 lg:text-lg ${isMobileOpen ? 'visible translate-x-0' : 'invisible -translate-x-full'}`}>
    <div className="sidebar-scroll shrink-0 overflow-hidden px-4 pb-3 pt-4">
      <div className="flex items-center justify-between gap-2"><button type="button" onClick={onBack} className="inline-flex min-h-11 items-center gap-2 rounded-lg px-3 font-semibold hover:bg-stone-deep"><ArrowLeft aria-hidden="true" className="size-4" />Back</button><button type="button" aria-label="Close settings navigation" onClick={onClose} className="flex size-11 items-center justify-center rounded-lg hover:bg-stone-deep lg:hidden"><X aria-hidden="true" className="size-5" /></button></div>
      <h2 className="my-4 px-3 font-display text-2xl leading-tight">Settings &amp;<br />Personalization</h2>
    ...
    <div className="sidebar-scroll min-h-0 flex-1 overflow-y-auto px-4 pb-6">
      {query.trim() ? <div><p role="status" className="mb-3 px-2 text-sm text-muted">{results.length ? `${results.length} ${results.length === 1 ? 'setting' : 'settings'} found` : 'No settings found. Try a different word.'}</p><ul className="space-y-1">{results.map(result => <li key={result.id}><button type="button" data-search-result aria-label={`${result.label}, ${SettingsSections.find(value => value.id === result.section)?.label}`} onClick={() => onNavigate(result.section, result.id)} className="w-full rounded-lg px-3 py-3 text-left transition hover:bg-stone-deep"><span className="block font-semibold">{result.label}</span><span className="mt-1 block text-sm text-muted">{SettingsSections.find(value => value.id === result.section)?.label}</span></button></li>)}</ul></div>
        : <nav aria-label="Settings sections"><div role="tablist" aria-label="Settings sections" aria-orientation="vertical" className="flex flex-col gap-1">{SettingsSections.map((item, index) => <a key={item.id} id={`settings-tab-${item.id}`} href={`/settings/${item.id}`} role="tab" aria-selected={section === item.id} aria-controls="settings-panel" tabIndex={section === item.id ? 0 : -1} onKeyDown={event => moveTab(event, index)} onClick={event => { if (!event.ctrlKey && !event.metaKey && !event.shiftKey && !event.altKey) { event.preventDefault(); onNavigate(item.id) } }} className={`flex min-h-12 items-center rounded-lg border-l-2 px-3 py-2.5 font-medium transition focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-pomegranate ${section === item.id ? 'border-pomegranate bg-stone-deep text-ink' : 'border-transparent text-ink-soft hover:bg-stone-deep/40'}`}>{item.label}</a>)}</div></nav>}
    </div>
  </aside>
}
