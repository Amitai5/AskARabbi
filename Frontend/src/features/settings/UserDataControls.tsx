...
export function UserDataControls({ isBusy, onDeleteChats, onDeleteAccount }: UserDataControlsProps) {
  ...
  return <>
    <div className="overflow-hidden rounded-xl border border-line bg-paper/60">
      <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="max-w-[27rem]">
          <h3 className="text-sm font-semibold">Delete all chats</h3>
          <p className="mt-1 text-sm leading-6 text-muted">Clear your entire conversation history and messages. Your account, preferences, and current usage stay the same.</p>
        </div>
        <button type="button" disabled={isBusy} onClick={() => setAction('chats')} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-lg border border-line-strong px-4 text-sm font-semibold transition hover:border-pomegranate/40 hover:bg-pomegranate/5 hover:text-pomegranate disabled:cursor-wait disabled:opacity-50"><Trash2 aria-hidden="true" className="size-4" />Delete all chats</button>
      </div>
      <div className="flex flex-col gap-4 border-t border-line bg-pomegranate/[0.025] p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="max-w-[27rem]">
          <h3 className="text-sm font-semibold">Delete account</h3>
          <p className="mt-1 text-sm leading-6 text-muted">Permanently delete your AskRabbi account, chats, personal profile, preferences, usage history, and sign-in identity.</p>
        </div>
        <button type="button" disabled={isBusy} onClick={() => setAction('account')} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-lg border border-pomegranate/35 px-4 text-sm font-semibold text-pomegranate transition hover:bg-pomegranate/10 disabled:cursor-wait disabled:opacity-50"><UserRoundX aria-hidden="true" className="size-4" />Delete account</button>
      </div>
    </div>
    <p className="mt-3 text-xs leading-5 text-muted">These actions cannot be undone. Deleting AskRabbi does not delete your Google account. Service backups and security logs follow separate retention policies.</p>
    {isBusy ? <p className="mt-3 text-sm text-ink-soft" role="status">Wait for your current answer or account update to finish before deleting data.</p> : null}
    {chatsDeleted ? <p className="mt-3 text-sm font-semibold text-ink" role="status">All chats deleted. Your account and settings were kept.</p> : null}
