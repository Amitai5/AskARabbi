...
export interface CalendarClient {
  ...
  getCachedOverview?(days: CalendarRange): CalendarOverview | null
  invalidate?(): void
}
...
export function createBackendCalendarClient(apiClient: ApiClient = createApiClient()): CalendarClient {
  return {
    getOverview: (days, signal) => apiClient.request(`/api/calendar/overview?days=${days}&includeAllCategories=true`, { signal, cache: 'no-store' }),
