...
export function CalendarPage({ client, onOpenDvarTorah, onBackToConversation }: Props) {
  ...
  return <section className="calendar-page min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-7 text-ink sm:px-8 sm:py-10 xl:px-10" aria-labelledby="calendar-title">
    <div className="mx-auto max-w-[82rem]">
      ...
      {data ? <>
        <div className="grid gap-7 border-b border-line py-7 xl:grid-cols-2">
        ...
        <div className="mt-7 grid items-start gap-8 xl:grid-cols-[minmax(0,1fr)_minmax(17rem,30%)]">
          ...
          <aside className="row-start-1 rounded-xl border border-line p-5 xl:col-start-2 xl:row-start-auto" aria-label="Shabbat and local times">
            <section><h2 className="font-display text-2xl">This Shabbat</h2><p className="mt-3">{formatCivilDate(data.shabbat.shabbatDate)}</p><p className="mt-2 text-sm leading-6 text-ink-soft">{data.shabbat.parashah ? `Parashat ${data.shabbat.parashah}` : `Festival reading · ${data.shabbat.holiday ?? 'No regular weekly portion'}`}</p><p className="mt-1 text-xs text-muted">{data.shabbat.inIsrael ? 'Israel' : 'Diaspora'} reading cycle</p><button type="button" onClick={onOpenDvarTorah} className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-lg border border-pomegranate px-4 text-sm font-semibold text-pomegranate hover:bg-pomegranate/5"><BookOpenText className="size-4" />Read Dvar Torah</button><p className="mt-2 text-xs leading-5 text-muted">Browse our published teachings; the publication’s own date and cycle apply.</p></section>
            <section className="mt-6 border-t border-line pt-6"><h2 className="flex items-center gap-2 font-display text-2xl"><MapPin className="size-5" />Local times</h2><AvailabilityNotice value={data.timing} onRetry={data.preferences.location && data.preferences.showLocalTimes ? retry : undefined} disabled={!isOnline} />
