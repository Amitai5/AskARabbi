...
export const UserMessage = memo(function UserMessage({ message }: UserMessageProps) {
  return (
    <div id={questionElementId(message.id)} tabIndex={-1} className="conversation-message reading-user-message flex justify-end rounded-lg" data-message-role="user">
      <div className="max-w-[88%] rounded-2xl rounded-br-md border border-line-strong/80 bg-stone px-4 py-3 shadow-sm sm:max-w-[78%] sm:px-5 sm:py-4">
        ...
        <p dir="auto" className="whitespace-pre-wrap text-base leading-7 text-ink sm:text-lg">{normalizeDisplayText(message.content)}</p>
      </div>
    </div>
  )
})
