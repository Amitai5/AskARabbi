...
describe('Background conversation navigation', () => {
  ...
  it.each([false, true])('retains a completed answer and its sources after closing and signing in again (follow-up: %s)', async (isFollowUp) => {
    ...
    expect(screen.getAllByRole('button', { name: 'Copy text' })).toHaveLength(isFollowUp ? 2 : 1)
    const sourceCopyButtons = screen.getAllByRole('button', { name: 'Copy with sources' })
    expect(sourceCopyButtons).toHaveLength(isFollowUp ? 2 : 1)
    await user.click(sourceCopyButtons[isFollowUp ? 1 : 0])
    const copied = await navigator.clipboard.readText()
    expect(copied).toContain('[1] Mishnah Chullin 8:1')
    expect(copied).toContain('https://www.sefaria.org/Mishnah_Chullin.8.1')
    expect(copied).toContain('Attribution: https://example.test/mishnah-yomit-attribution')
