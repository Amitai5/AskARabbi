...
The normal writing prompt contains only calculated age, name, optional bio, optional religious background, and Jewish heritage—not the exact birth date, time, or time zone. When a calendar question requires the saved date, trusted process-local code performs the calculation and only the derived result and its assumptions return as citable evidence. The answer never describes that internal mechanism. `GroundedAnswerService` marks all ordinary profile fields as untrusted personalization context, does not add them to retrieval keywords, and instructs the model not to stereotype, infer observance, or claim profile-specific rules without supporting textual evidence. Heritage may guide harmless language choices such as `Tevet` for Sephardi or Mizrahi readers and `Teves` for Ashkenazi readers; it never establishes a legal conclusion.
...
Every model-facing instruction, the strict response schema, and the legacy compatibility notice live in the tracked [`Prompts`](Prompts) directory. They are copied into build and publish output, loaded only when AI Chat or `ask` is used, and validated before the first model request. The library receives them as a `GroundedPromptSet`; it no longer contains hidden default prompt text. Current renderers do not display the compatibility notice.
...
`ask` can answer independently reviewed background or uncertainty even when textual evidence is absent or tangential. Useful regression questions include `Who was Rabbi Akiva?` and `In a fictional story, is a vampire alive or dead?`. It fails closed with a nonzero exit code if the index is stale, Azure is unavailable, or citation, quotation, relevance, or claim-support validation fails after one repair. A successful answer normally uses one model request to draft the answer and a second structured request to audit every claim according to its Source, Background, or Uncertainty kind. Console output includes the answer, exact evidence packet, and trace without quote panels, a repeated source bibliography, diagnostic limitations, or a stock disclaimer. JSON output retains the legacy interpretive-notice property for contract compatibility, but conversational renderers ignore it. The API's fixed, saved recovery reply belongs to `GroundedConversationTurnService`; the console does not translate a failed library result into success. See [answer reliability](../docs/ANSWER_RELIABILITY.md) for claim boundaries and the host-specific outcomes.
