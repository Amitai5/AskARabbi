...
Each account has **10,000,000 tokens per UTC calendar month**. Settings and the chat composer display the consumed percentage, not a question count. The server remains authoritative even if a browser is stale or bypasses the UI.

## Configuration

- Production default: `Backend/AskARabbi.Api/appsettings.Production.json`, `Usage:MonthlyTokenLimit`.
- Environment override: `Usage__MonthlyTokenLimit=10000000`.
...
All three defaults agree at 10M: application options, production settings, and the example configuration. A non-positive limit fails startup validation.
...
An already-admitted provider response can finish slightly above the allowance, because its exact usage is only available afterward. The actual total is retained, the UI clamps at 100%, and no further provider request or question is admitted. This is an admission limit, not a promise that a paid response is terminated at exactly token 10,000,000. A provider timeout without returned usage cannot be accurately charged from an estimate.
