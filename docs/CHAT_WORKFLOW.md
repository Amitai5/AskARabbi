...
The profile contributes a preferred-name token, a broad child/teenager/adult audience group, optional learning context, optional self-described religious background, and self-described Jewish heritage. The exact date and time of birth and birth time zone are omitted from the normal model prompt. When a birthday calculation is requested, trusted server code can use those saved fields and return only the derived calendar result and its assumptions. The answer states that result naturally and never exposes how the application produced it. Current response and quotation language choices are applied independently to drafting, repair, auditing, and presentation; see [conversation personalization](PERSONALIZATION.md) for the complete setting map and fallback rules.
