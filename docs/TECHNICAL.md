...
- A weekly-learning destination separate from conversation history.
...
Azure Cosmos DB for MongoDB is accessed behind `IUserAccountStore`, `IConversationStore`, `IConversationSettingsStore`, `IUsageStore`, and `IWeeklyDvarTorahStore`. Conversation metadata and messages occupy separate collections so sidebar reads avoid message bodies and appends do not rewrite an ever-growing document. Personalization and conversation preferences share an owner-keyed settings document but use independent field updates. Store filters always include the local user ID for user-owned resources. Global weekly publications instead use a deterministic reading-cycle/Shabbat key; a separate `IWeeklyDvarTorahGenerationStore` grants one expiring lease and changes a document to `Published` atomically. Required uniqueness/navigation/current-publication indexes are initialized at configured startup.
...
| `GET /api/dvar-torah` | Return the upcoming Shabbat and its current publication, or the latest earlier fallback while current content is pending |
