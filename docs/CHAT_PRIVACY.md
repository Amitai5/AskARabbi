...
The sign-in screen has a short, always-visible notice linking to **Terms of Use** (`https://askarabbi.ai/terms`) and **Privacy Policy** (`https://askarabbi.ai/privacy`); **Settings & Personalization → Your data** shows the same notice under **Terms and privacy** beside the deletion controls. Both render `Frontend/src/components/ChatPrivacyNotice.tsx` without new API calls. The links open in a new tab to preserve the current form or conversation, with that behavior announced to assistive technology. They replace the expandable retention explanation rather than adding a consent checkbox or making new retention promises. The policy documents are owned and deployed by `WebsiteFrontend`; the app uses absolute links and preserves policy section anchors. The storage boundaries documented above and the deletion safeguards remain unchanged.
