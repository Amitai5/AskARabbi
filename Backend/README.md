...
Three milestone-specific production dependencies are pinned: `WorkOS.net` 6.2.0 in the API, plus `MongoDB.Driver` 3.11.0 and `Zmanim` 1.5.0 in the reusable library. WorkOS and MongoDB avoid custom authentication and wire-protocol clients. Zmanim supplies the weekly parashah schedule while .NET supplies numeric Hebrew-calendar conversion; it adds one small managed assembly with no .NET Standard 2.0 transitive package dependencies. These integrations stay behind narrow services and must remain covered by dependency updates and security scanning. See [`THIRD_PARTY_NOTICES.md`](../THIRD_PARTY_NOTICES.md) for Zmanim's LGPL notice.
