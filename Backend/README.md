...
`POST /api/conversations` creates a saved conversation together with its first user message; opening or abandoning an empty browser draft never writes to Cosmos DB. That first turn and `POST /api/conversations/{conversationId}/messages` check the current allowance, retrieve only approved Sefaria evidence through a forced Azure OpenAI Responses `file_search` call, expose three bounded local calendar functions when relevant, generate and audit a strict structured draft, persist reviewed assistant text with any trusted quotation/context/provenance snapshots or a fixed localized recovery reply, and record provider-reported input and output token usage even when an answer fails validation. The functions convert a supplied or privately loaded birth date to a Hebrew date, find the weekly parashah or festival-displaced reading for a week or Hebrew birthday anniversary, and return today's Gregorian and Hebrew dates. A reviewed first response can supply a concise AI-generated title, which the backend applies once. A first-turn recovery keeps the default title and does not reuse any rejected title; later replies do not regenerate it. Retrieval ignores model prose, resolves provenance through the bundled checksum-validated manifest, and reapplies source filters locally. An empty or irrelevant ordinary search result can still produce independently reviewed background or uncertainty. `ValidationFailed` and `InsufficientEvidence` are recovered as a saved assistant reply; provider failures, unavailable retrieval, cancellation, and usage failures retain their distinct outcomes. Compound parashah requests retain their required-coverage preflight.
...
Warm answer requests use one bounded managed-corpus search, up to 20 candidates, at most 10 evidence segments, medium answer-model reasoning, low audit-model reasoning, and separate 8,000-token answer and 3,200-token audit budgets. Successful retrievals are cached in process for 10 minutes by normalized query and source filters. The independent review remains mandatory. Basic biography, introductory background, and explicit fiction may pass without Torah citations; religious rulings, attributed teachings, quotations, and calculated dates still require verified evidence. If support is insufficient or both drafts fail validation, the API saves a localized uncertainty reply without rejected content or citations and returns the existing `answered` status. The trace retains the failure and actual token usage; provider outages retain their existing error statuses. Account-scoped monthly admission precedes paid AI work. Each provider response updates the shared token counter, and successful titles are saved with the answer. Responses expose `Server-Timing` entries for the complete turn, retrieval, and model work. See [answer reliability](../docs/ANSWER_RELIABILITY.md) for claim kinds, the recovery/status table, idempotency, diagnostics, and compatibility.
...
The non-secret performance settings can also be overridden with `AI__MaximumOutputTokens`, `AI__ValidationMaximumOutputTokens`, `AI__ReasoningEffort`, `AI__ServiceTier`, `AI__MaximumCandidates`, `AI__MaximumEvidenceSegments`, `AI__MaximumEvidenceCharacters`, `AI__MaximumEnrichmentHits`, `AI__RecentConversationTurns`, `AI__RetrievalCacheSeconds`, and `AI__RetrievalCacheMaximumEntries`. Production conversations request the `Priority` service tier for file-search retrieval, answer generation, and the independent grounding audit. Answer generation uses medium reasoning with an 8,000-token combined reasoning-and-structured-output ceiling; the smaller audit remains low reasoning. Azure can fall back to standard processing when priority capacity is unavailable. Change these settings only after measuring answer quality, latency, and cost together.
...
| `POST /api/conversations/{id}/messages` | Stores one user message by client idempotency ID and returns canonical context plus a turn status. Reviewed answers and fixed recovery replies return `answered`; sources may be empty. All reported chat token usage is counted. Add `?compact=true` for the bounded current-turn response used by the frontend. |
...
[`AskARabbi.DvarTorahJob`](AskARabbi.DvarTorahJob) is the separate one-shot writer image. The `askarabbi-dvar-torah-production` Azure Container Apps Job shares the VNet-integrated `askarabbi-containerapps-production` environment with `askarabbi-api-production`. It is scheduled with `5 8 * * 0`, which Azure evaluates as Sunday at 08:05 UTC. Each execution reads the Cosmos Mongo connection string from a job-level secret reference, calculates the upcoming Shabbat with the same pinned calendar service as the API, checks for the exact published week before creating generation dependencies, acquires a recoverable Mongo lease if needed, and atomically publishes once. Platform retries either return `AlreadyPublished`, observe `GenerationInProgress`, or recover an expired lease.
