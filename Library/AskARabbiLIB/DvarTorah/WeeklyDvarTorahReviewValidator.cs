...
internal static class WeeklyDvarTorahReviewValidator
{
    internal static IReadOnlyList<string> Validate(WeeklyDvarTorahReviewDraft review, ICollection<string>? failureCodes = null, IReadOnlyList<string>? allowedEvidenceIds = null)
    {
        ...
        AddFailure(review.OpeningHookGrounded, "Independent review found no engaging, grounded modern opening hook. Open with the corroborated development or a clearly imagined everyday scenario, then bridge to the Torah question without inventing facts or statistics.", errors, failureCodes, nameof(review.OpeningHookGrounded));
        AddFailure(review.QuotationsIntegrated, "Independent review found quotations disconnected from the argument. Introduce and explain each exact quotation within its paragraph, in natural spoken sentences.", errors, failureCodes, nameof(review.QuotationsIntegrated));
        AddFailure(review.HookTorahBridgeNatural, "Independent review found an abrupt hook-to-Torah transition. Before the first quotation, carry the hook's concrete human decision into the Torah scene and textual question with explicit bridging sentences; a shared topic or generic transition is not enough.", errors, failureCodes, nameof(review.HookTorahBridgeNatural));
        AddFailure(review.SpokenFlowNatural, "Independent review found disconnected or unnatural spoken flow. Revise the connected passage so each paragraph develops the previous idea. Speak about people and choices directly instead of describing the text's argument in academic language or listing leftover verses. Check sentence cadence with citations silent and exact quotations unchanged.", errors, failureCodes, nameof(review.SpokenFlowNatural));
