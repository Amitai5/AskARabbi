...
`AskARabbiLIB` is the reusable .NET 10 library for manifest search, checksum-verified source access, segment indexing/retrieval, Azure AI access, bounded local AI tools, Hebrew-calendar calculations, weekly Dvar Torah publication orchestration, optional Key Vault access, typed conversational grounding and independent answer validation, production account/conversation rules, and Azure Cosmos DB for MongoDB persistence. Its namespace, project, assembly, and solution are all named `AskARabbiLIB`.
...
2. For an explicit bar- or bat-mitzvah question that asks both which portion applies and what it is about, resolve the reading first. Search every canonical chapter anchor plus a semantic story query in parallel, search only the enabled Torah collection, and accept only canonical references inside that portion's range. Require representative coverage across at least three references before generation. This avoids asking the model to describe a reading that was unknown when retrieval ran, relying on a transliterated name to exist in the text, or attempting a whole-story summary from one isolated verse.
3. For other questions, retrieve at most 50 local candidates using the current question plus limited recent user context.
4. Apply a deterministic topic-and-support adequacy gate. Empty or tangential retrieval leaves an empty packet instead of blocking generation. The writer may provide independently reviewed basic biography or introductory background, acknowledge uncertainty, or use registered research capabilities; religious rulings, attributed teachings, quotations, and calendar calculations still require verified evidence.
5. Build at most 24 evidence items and 48,000 text characters by default, with document diversity, up to six neighboring segments on each side, canonical-reference translation pairing, and explicit excerpt labels. The nine-segment per-document cap normally yields the cited segment, up to six preceding segments, and two following segments when all are available.
6. Send only the bounded packet, recent process-memory context, and a minimized profile context through `IAIEngine`. The normal prompt contains calculated age rather than date of birth. For a relevant calendar request not already resolved by the compound-question preflight, the model may call one of three bounded local functions; omitting the birth-date argument lets trusted server code use the saved date privately without putting it in the prompt or tool output.
7. Convert every successful calendar result into request-local evidence with an opaque evidence ID and exact text. Calendar results must be cited and quoted like corpus evidence, but the user-facing answer states the result naturally and never exposes functions, tools, calls, retrieval, prompts, models, providers, or other implementation mechanisms.
8. Require strict structured claims with a `Source`, `Background`, or `Uncertainty` kind. Source claims and disagreements require evidence IDs and exact quotations; Background and Uncertainty require empty evidence and quotation arrays and null attribution.
9. Reject unknown evidence IDs, uncited Source statements, unquoted cited sources, and quotations that are not exact substrings of the trusted source segment or calculated result. A claimed interpretation chain therefore needs exact passages for both the later view and its earlier textual basis.
10. Run an independent structured audit of every claim and disagreement. The audit checks relevance and support appropriate to each claim kind: textual entailment for Source, accuracy and restricted scope for Background, and honest limits for Uncertainty. It rejects unrelated legal analogies or unsupported modern rulings even when their quotations are authentic.
11. Materialize every title, reference, edition, language, license, file path, and original-source URL from trusted application objects, never model output. Attribution-required sources render as trusted clickable links, and redirected output exposes the equivalent Markdown link.
12. Permit one repair after either validation layer, including remaining bounded source research. Preserve a validation failure internally if repair fails; the web API saves an application-owned, localized uncertainty reply without rejected prose or citations, using the same idempotent assistant-message ID. Render only validated conversational content: diagnostic limitation fields and the legacy interpretive-notice property are not appended to the displayed answer.
...
The internal `GroundedClaimKind` contract does not add fields to public `GroundedAnswer` or saved messages. Older/custom internal drafts default to `Source`; production structured output requires an explicit kind. The auditor can reconcile Source citations against all supplied evidence, while Background and Uncertainty remain uncited. Valid-sized hidden limitation notes about internal mechanisms are discarded; visible prose still rejects those details. See [answer reliability](../docs/ANSWER_RELIABILITY.md) for the complete contract, failure table, and regression cases.

The host supplies a validated `GroundedPromptSet` to `GroundedAnswerService`. Every instruction template, strict JSON schema, and legacy compatibility notice is loaded from the tracked [`Prototype/Prompts`](../Prototype/Prompts) catalog. The API copies that same catalog into its published output; deploy it with the matching library. This keeps behavior changes visible and reviewable while the reusable library remains independent of any particular filesystem layout.

The API also registers bounded `search_bdb_dictionary` and `read_bdb_entry` functions. They use a pinned original dictionary in MongoDB, not model-generated definitions or an external dictionary API. Original excerpts receive independent evidence IDs and undergo the same exact-quotation and semantic-support validation as other evidence. Dictionary English is not mistaken for a user's Torah quotation-language choice; explanatory prose still follows personalization. The API only reads published editions. The explicit importer creates the shared collection and indexes; API startup does not import or index dictionary data. This capability supports linguistic research, not an unsourced bridge from a biblical dictionary to later customs or halakhic rulings.

## Primary contracts

- AI: `IAIEngine`, `AIEngineOptions`, `AIMessage`, `AIEngineResult<T>`, `AIUsage`, `AzureOpenAIEngine`.
- AI tools: `AIToolAttribute`, `AIToolParameterAttribute`, `IAIToolRegistry`, `AIToolRegistry`, `AIToolExecutionSession`, `CalendarAITools`, and `BdbDictionaryAITools`.
- Local dictionaries: `ILexiconStore`, `LexiconEntry`, `BdbEntryParser`, `BdbCorpus`, and `MongoLexiconStore`. See the [BDB importer](../Tools/AskARabbi.DictionaryImporter/README.md).
