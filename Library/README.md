...
4. Apply a deterministic topic-and-support adequacy gate. A normal textual question returns `InsufficientEvidence` without calling a model when retrieval is empty or merely tangential. A recognized Hebrew-calendar question may continue with only the explicitly registered local calendar tools available.
