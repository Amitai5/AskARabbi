...
2. For an explicit bar- or bat-mitzvah question that asks both which portion applies and what it is about, run the deterministic calendar calculation first. Seed retrieval with canonical reference anchors for the resolved parashah, search only the enabled Torah collection, and accept only canonical references inside that portion's range. This avoids asking the model to describe a reading that was unknown when retrieval ran or relying on the corpus to contain the parashah's transliterated name.
...
7. Convert every successful tool result into request-local calculated evidence with an opaque evidence ID and exact text. Calendar calculations must be cited and quoted like corpus evidence, but remain visibly identified as calculations rather than source texts or religious rulings.
...
12. Permit one same-evidence repair after either validation layer; fail visibly if validation still fails. Materialize the host-supplied, validated interpretive notice without asking the provider to generate or modify it.
