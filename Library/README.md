...
4. Apply a deterministic topic-and-support adequacy gate. A normal textual question returns `InsufficientEvidence` without calling a model when retrieval is empty or merely tangential. Recognized calendar or lexical questions may continue with the explicitly registered local capabilities even when religious retrieval is insufficient; a lookup must supply actual evidence before an answer can pass validation.
...
8. Require strict structured claims, evidence IDs, attributed perspectives, disagreements, mandatory quotations for every cited evidence ID, quotation roles, limitations, a clarifying question, and a human-guidance flag.
9. Reject unknown evidence IDs, uncited statements, unquoted cited sources, and quotations that are not exact substrings of the trusted source segment or calculated result. A claimed interpretation chain therefore needs exact passages for both the later view and its earlier textual basis.
10. Run an independent structured audit of every claim and disagreement. The audit checks relevance to the question and entailment from the cited passages or deterministic result, and rejects unrelated legal analogies or unsupported modern rulings even when their quotations are authentic.
...
12. Permit one same-evidence repair after either validation layer; fail visibly if validation still fails. Render only validated conversational content: diagnostic limitation fields and the legacy interpretive-notice property are not appended to the displayed answer.
